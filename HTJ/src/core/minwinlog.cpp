...
// Function to read and display events from the Windows System event log
void readSystemEventLog() {
    ...
    while (ReadEventLog(hEventLog, EVENTLOG_BACKWARDS_READ | EVENTLOG_SEQUENTIAL_READ, 0, buffer, sizeof(buffer), &dwBytesRead, &dwRead)) {
        ...
        while ((BYTE*)pEvent < buffer + dwBytesRead) {
            ...
            std::cout << "Source: " << (char*)((BYTE*)pEvent + pEvent->StringOffset) << std::endl;
            std::cout << "Message: " << (char*)((BYTE*)pEvent + pEvent->StringOffset + strlen((char*)((BYTE*)pEvent + pEvent->StringOffset)) + 1) << std::endl;
            std::cout << std::endl;
